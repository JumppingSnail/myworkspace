<template>
    <div class="hello">
        ...
        <div class="info-box" :style="is_weixin?'top:46px;':'top:46px;'">
            ...
            <div class="operation">
                ...
                <div>
                    ...
                    <img src="../../../static/img/bookmark.png"  @click="bookmark" alt="">
                    <img src="../../../static/img/menu.png" @click="menuBtn" alt="">
                </div>
            </div>
        </div>
        <div class="content" :style="is_weixin?'padding-top: 137px;':'padding-top: 137px;'">
			...
			<van-popup v-model="show" position="right" :title="title" >
                <div class="menuContent">
                    <van-tabs v-model="active" sticky>
                        ...
                        <van-tab title="书签">
                            <div class="bookmark" v-for="(item,index) in bookmarkList" :key="index">
                                <p class="book-title">{{ item.create_time.substring(0,19).replace('T',' ') }}</p>
                                <p class="book-info" @click="getMarkSectionContent(index)">{{ item.mark_content }}</p>
                            </div>
                        </van-tab>
                    </van-tabs>
                </div>
		...
		methods: {
			...
			getData () {
				...
				this.$post('front/fetchKnowledgeContent.do', {
					'reposId': this.id,
					'sectionId': this.menuList.length > 0 ? this.menuList[this.menuActive].sectionId : ''
				}).then((e) => {
					if (e.result === 'ok') {
            ...
            // 获取书签列表
			getBookmarkList () {
				this.$post('front/getMarkList.do', {
					'reposId': this.id,
				}).then((e) => {
					if (e.result === 'ok') {
						this.bookmarkList = e.data
					} else {
						this.$toast('服务器异常，请联系管理员');
					}
				}).catch(() => {
